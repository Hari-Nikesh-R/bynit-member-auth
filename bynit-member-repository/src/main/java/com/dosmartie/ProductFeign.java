...
//    ResponseEntity<BaseResponse<String>> addProduct(@RequestBody @Valid ProductCreateRequest product,);
//
//    @GetMapping(value = ProductUrls.PRODUCT + "/{category}")
//    ResponseEntity<BaseResponse<List<ProductResponse>>> getProductViaCategory(@PathVariable("category") String category, @RequestParam(PAGE) int page, @RequestParam(SIZE) int size, @RequestParam(value = MERCHANT, required = false) String merchant, );
//
//    @GetMapping(value = ProductUrls.PRODUCT)
//    ResponseEntity<BaseResponse<List<ProductResponse>>> getProduct(@RequestParam(SEARCH_PARAM) String searchParam, @RequestParam(PAGE) int page, @RequestParam(SIZE) int size, @RequestParam(value = MERCHANT, required = false) String merchant,);
//
//    @DeleteMapping(value = ProductUrls.PRODUCT + "/variant")
//    ResponseEntity<BaseResponse<String>> deleteProductVariant(@RequestParam(VARIANT_SKU) String itemSku,, @RequestHeader(MERCHANT) String email);
//
//    @DeleteMapping(value = ProductUrls.PRODUCT)
//    ResponseEntity<BaseResponse<String>> deleteProduct(@RequestParam(DEFAULT_SKU) String itemSku,, @RequestHeader(MERCHANT) String email);
//
...
//    BaseResponse<List<ProductResponse>> getAllProducts(@RequestParam(PAGE) int page, @RequestParam(SIZE) int size, @RequestParam(value = MERCHANT, required = false) String merchant,);
//
//
//    @PostMapping(value = ProductUrls.PRODUCT + ProductUrls.STOCK)
//    ResponseEntity<BaseResponse<String>> updateStock(@RequestBody CartProductRequest cartProductRequest,);
//
//    @PutMapping(value = ProductUrls.PURCHASE + ProductUrls.RATE)
//    ResponseEntity<BaseResponse<?>> rateProduct(@RequestBody @Valid OrderRatingRequest orderRatingRequest,, @RequestHeader("email") String email);
